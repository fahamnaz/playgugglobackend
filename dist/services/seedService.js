...
import { childProfileSeed, dashboardSummarySeed, mathEquationSeeds, notificationSeeds, planetSeeds, signLevelSeeds, speechLevelSeeds, subjectSeeds, wordChallengeSeeds, } from '../seed/seedData.js';
...
export async function seedDatabase() {
    ...
    const childProfile = await ChildProfile.findOneAndUpdate({ slug: childProfileSeed.slug }, { $set: childProfileSeed }, { upsert: true, new: true, setDefaultsOnInsert: true });
    await DashboardSummary.findOneAndUpdate({ childSlug: dashboardSummarySeed.childSlug }, {
        $set: {
            ...dashboardSummarySeed,
            childId: childProfile._id,
        },
    }, { upsert: true, new: true, setDefaultsOnInsert: true });
    await Promise.all(notificationSeeds.map((notification) => Notification.findOneAndUpdate({ seedKey: notification.seedKey }, {
        $set: {
            ...notification,
            childId: childProfile._id,
        },
